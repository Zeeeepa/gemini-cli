...
describe('mcp-client', () => {
  ...
  describe('dicoverTools', () => {
    it('should discover tools via mcpServerCommand', async () => {
      ...
      const mockedMcpToTool = vi.mocked(GenAiLib.mcpToTool).mockReturnValue({
        tool: () => ({
          functionDeclarations: ["testFunction"],
        }),
      } as unknown as GenAiLib.CallableTool);

      const tools = await dicoverTools("test-server", {}, mockedClient);
  ...
  describe('appendMcpServerCommand', () => {
    it('should do nothing if no MCP servers or command are configured', () => {
      const out = appendMcpServerCommand({}, undefined);
      expect(out).toEqual({});
    });

    it('should discover tools via mcpServerCommand', () => {
      const commandString = 'command --arg1 value1';
      const out = appendMcpServerCommand({}, commandString);
      expect(out).toEqual({
        mcp: {
          command: 'command',
          args: ['--arg1', 'value1'],
    ...
    it('should handle error if mcpServerCommand parsing fails', () => {
      expect(() => appendMcpServerCommand({}, 'derp && herp')).toThrowError();
    });
  });
  ...
  describe('createTransport', () => {
    ...
    it('should connect via url', async () => {
      const transport = createTransport(
        {
    ...
    it('should connect via command', () => {
      const mockOn = vi.fn();
      // mock the SdkClientStdioLib.StdioClientTransport constructor
      const mockedTransport = vi
        .mocked(SdkClientStdioLib.StdioClientTransport)
        .mockImplementation(
          () =>
            ({
              stderr: {
